...
AS $$
  WITH active_matches AS (
    SELECT fs.id, public.is_fee_structure_complete(fs.id) AS is_complete
  ...
  SELECT
    (SELECT id FROM active_matches WHERE is_complete ORDER BY id::text LIMIT 1) AS structure_id,
    (SELECT COUNT(*)::integer FROM active_matches) AS match_count;
$$;
...
BEGIN
  ...
  IF COALESCE(matches, 0) = 0 OR selected_structure_id IS NULL THEN
...
REVOKE EXECUTE ON FUNCTION public.find_complete_fee_structure(uuid, uuid) FROM PUBLIC, anon, authenticated;
