...
function GradeScalesPage() {
  ...
  const addScale = useMutation({
    ...
    onSuccess: () => {
      ...
      setName("");
      qc.invalidateQueries({ queryKey: ["exam-grade-scales"] });
    },
  ...
  const delScale = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["exam-grade-scales"] });
      if (activeScaleId) setActiveScaleId(null);
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Scales</CardTitle>
        </CardHeader>
                ...
                scales.data.map((s) => (
                  <TableRow key={s.id} className={activeScaleId === s.id ? "bg-muted/40" : ""}>
                    ...
                    <TableCell className="text-right space-x-2">
                      <Button size="sm" variant="outline" onClick={() => setActiveScaleId(s.id)}>
                        Manage Bands
                      </Button>
                      {canManageExams && (
                        <Button size="sm" variant="ghost" onClick={() => delScale.mutate(s.id)}>
...
function BandsEditor({ scaleId, canEdit }: { scaleId: string; canEdit: boolean }) {
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("exam_grade_bands").insert({
        scale_id: scaleId,
        grade: grade.trim(),
        min_percent: min,
        max_percent: max,
    ...
    onSuccess: () => {
      setGrade("");
      setRemark("");
      qc.invalidateQueries({ queryKey: ["exam-grade-bands", scaleId] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
