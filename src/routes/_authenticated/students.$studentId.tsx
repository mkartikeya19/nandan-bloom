  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: recs, error: rErr } = await supabase
        ...
        .select(
          "*, academic_sessions:academic_session_id(name), school_classes:class_id(name), school_sections:section_id(name), houses:house_id(name)",
        )
    ...
    <div>
        ...
        actions={
          <div className="flex gap-2">
            ...
            {perms.canPromoteStudent && (
              <Button variant="outline" onClick={() => setPromote(true)}>
      ...
      <Tabs defaultValue="info">
        ...
        <TabsContent value="info">
          <Card>
            <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-sm">
              ...
              <Info
                label="Guardian"
                value={`${s.guardian_name ?? "—"} • ${s.guardian_phone ?? "—"}`}
              />
              <Info
                label="Emergency"
                value={`${s.emergency_contact_name ?? "—"} • ${s.emergency_contact_number ?? "—"}`}
              />
              <Info
                label="Address"
                  ...
                  {records.length === 0 ? (
                    ...
                    records.map((r) => (
                      <TableRow key={r.id}>
                        ...
                        <TableCell>
                          {r.joined_on ? new Date(r.joined_on).toLocaleDateString("en-IN") : "—"}
                        </TableCell>
                        <TableCell>
        ...
        <TabsContent value="activity">
          <Card>
            <CardContent className="p-6 space-y-3 text-sm">
              ...
              {s.date_of_admission && (
                <ActivityItem
                  date={s.date_of_admission}
                  label={`Admitted (${s.admission_type ?? "—"})`}
                />
              )}
              {records
                .slice()
        ...
        <TabsContent value="fees">
          <StudentFeesTab
            studentId={studentId}
            activeRecordId={current?.id ?? null}
            hasFeeStructure={!!current?.fee_structure_id}
          />
        </TabsContent>
        ...
        <TabsContent value="attendance">
          <Card>
            <CardContent className="p-6 text-sm text-muted-foreground">
              Attendance summary will appear here once linked to this student.
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card>
            <CardContent className="p-6 space-y-2 text-sm">
              <DocLine
                ...
                path={s.photo_url}
                studentId={s.id}
                scholar={s.scholar_number}
                folder="photos"
                accept="image/*"
                canReplace={perms.canEditStudent}
              />
              <DocLine
                label="Birth Certificate"
                field="birth_certificate_url"
                path={s.birth_certificate_url}
                studentId={s.id}
                scholar={s.scholar_number}
                folder="documents"
                canReplace={perms.canEditStudent}
              ...
              <DocLine
                label="Aadhaar Copy"
                field="aadhaar_copy_url"
                path={s.aadhaar_copy_url}
                studentId={s.id}
                scholar={s.scholar_number}
                folder="documents"
                canReplace={perms.canEditStudent}
              ...
              <DocLine
                ...
                field="transfer_certificate_url"
                path={s.transfer_certificate_url}
                studentId={s.id}
                scholar={s.scholar_number}
                folder="documents"
                canReplace={perms.canEditStudent}
...
function DocLine({
  ...
  useEffect(() => {
    let alive = true;
    if (path) {
      getSignedStudentUrl(path).then((u) => {
        ...
        {canReplace && (
          <label className="inline-flex">
            ...
            <Button size="sm" variant="outline" asChild disabled={uploading}>
              <span>
                <Upload className="h-4 w-4" /> {path ? "Replace" : "Upload"}
              </span>
            </Button>
          </label>
        )}
