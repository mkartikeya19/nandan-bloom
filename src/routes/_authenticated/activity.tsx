...
function ActivityCenter() {
  ...
  const profileMap = useMemo(() => {
    ...
    (profiles ?? []).forEach((p) =>
      m.set(p.id, { name: p.full_name ?? p.email ?? "—", email: p.email ?? "", roles: [] }),
    );
    (roles ?? []).forEach((r) => {
  ...
  const filtered = (rows ?? []).filter((r) => {
    ...
    const hay = [r.module, r.action, r.entity_type, p?.name, p?.email]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
  ...
  return (
    <div>
      <PageHeader
        ...
        description={
          canSeeAll
            ? "Audit log of all important actions across the ERP."
            : "Audit log of your own actions."
        }
        actions={
                ...
                filtered.map((r) => {
                  ...
                  return (
                    <TableRow key={r.id}>
                      ...
                      <TableCell
                        className="text-xs text-muted-foreground max-w-[360px]"
                        title={JSON.stringify(r.details ?? {})}
                      >
                        {formatActivityDetails(r.module, r.action, r.details)}
                      </TableCell>
                    </TableRow>
                  );
                })
