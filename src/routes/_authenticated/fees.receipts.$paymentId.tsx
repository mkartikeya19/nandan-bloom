...
function ReceiptView() {
  ...
  const payment = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("fee_payments")
        .select(
          "*, students(scholar_number, full_name, father_name, student_academic_records(school_classes(name), school_sections(name), academic_sessions(name), status)), fee_payment_allocations(id, amount, student_fee_schedule(period_label, is_opening_balance, fee_heads(name)))",
        )
        .eq("id", paymentId)
        .single();
  ...
  const school = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("school_profile").select("*").limit(1).maybeSingle()).data,
  });
  ...
  const doPrint = () => {
    markPrinted.mutate("print");
    setTimeout(() => window.print(), 120);
  };
  const doDownload = () => {
    markPrinted.mutate("download");
    setTimeout(() => window.print(), 120);
  };
  ...
  const active =
    p.students?.student_academic_records?.find((r) => r.status === "Active") ??
    p.students?.student_academic_records?.[0];
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2 print:hidden">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {!p.is_void && canVoidReceipt && (
            <VoidReceiptButton
              onConfirm={(reason) => voidReceipt.mutate(reason)}
        ...
        <CardContent className="p-8 print:p-4">
          <div className="text-center border-b pb-4 mb-4">
            <h1 className="text-2xl font-bold">
              {school.data?.name ?? "Nandan Kids Higher Secondary School"}
            </h1>
            {school.data?.address && (
              <p className="text-sm text-muted-foreground">{school.data.address}</p>
            )}
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm mb-4">
            <div>
              ...
              <span className="font-mono font-semibold">{p.receipt_number}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Date &amp; Time:</span>{" "}
              {paidAt ? paidAt.toLocaleString("en-IN") : "—"}
            </div>
            <div>
              <span className="text-muted-foreground">Scholar No:</span>{" "}
              {p.students?.scholar_number}
            </div>
            <div>
              <span className="text-muted-foreground">Session:</span>{" "}
              {active?.academic_sessions?.name ?? "—"}
            </div>
            <div>
            ...
            <div>
              ...
              {active?.school_classes?.name ?? "—"}
            </div>
            <div>
              <span className="text-muted-foreground">Section:</span>{" "}
              {active?.school_sections?.name ?? "—"}
            </div>
          </div>
            ...
            <tbody>
              {allocations.map((a) => (
                <tr key={a.id} className="border-t">
                  <td className="p-2">
                    {a.student_fee_schedule?.is_opening_balance
                      ? "Opening Balance"
                      : (a.student_fee_schedule?.fee_heads?.name ?? "—")}
                  </td>
              ...
              <tr className="border-t bg-muted font-semibold">
                ...
                <td className="p-2 text-right">{formatINR(Number(p.amount))}</td>
              </tr>
            </tbody>
          </table>
          ...
          <p className="text-sm mb-4">
            ...
            <span className="italic">{amountInWords(Number(p.amount))}</span>
          </p>

          <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
            <div>
              <span className="text-muted-foreground">Payment Mode:</span> {p.payment_mode}
            </div>
            <div>
            ...
            <div>
              <span className="text-muted-foreground">Collected By:</span> {nameOf(p.collected_by)}
            </div>
            <div>
            ...
            {p.notes && (
              <div className="col-span-2">
                <span className="text-muted-foreground">Remarks:</span> {p.notes}
              </div>
            )}
            <div>
              <span className="text-muted-foreground">Outstanding After Payment:</span>{" "}
              <span className="font-semibold">{formatINR(outstandingAfter.data ?? 0)}</span>
            </div>
            <div>
              <span className="text-muted-foreground">Print Count:</span>{" "}
              {p.receipt_print_count ?? 0}
            </div>
            {p.is_void && (
              <>
                <div className="col-span-2">
                  <span className="text-muted-foreground">Void Reason:</span> {p.void_reason}
                </div>
                <div>
                  <span className="text-muted-foreground">Voided By:</span> {nameOf(p.voided_by)}
                </div>
                <div>
                  <span className="text-muted-foreground">Voided At:</span>{" "}
                  {p.voided_at ? new Date(p.voided_at).toLocaleString("en-IN") : "—"}
                </div>
              </>
            )}
          </div>
          ...
          <div className="mt-16 flex justify-between text-sm">
            <div className="text-center">
              <div className="border-t border-black w-40 mb-1" /> Parent Signature
            </div>
            <div className="text-center">
              <div className="border-t border-black w-40 mb-1" /> Cashier Signature
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            The receipt number is preserved and marked VOID. All allocations are reversed, the
            student ledger is restored, and this action is recorded in the Activity Log. To correct
            a mistake, void this receipt and post a new one.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder="e.g. Wrong amount entered, cheque bounced, duplicate receipt"
            rows={3}
          />
        </div>
        <AlertDialogFooter>
