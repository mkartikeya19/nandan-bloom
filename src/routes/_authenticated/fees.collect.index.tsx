  ...
  const results = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("students")
        .select(
          "id, scholar_number, full_name, father_name, student_academic_records(academic_session_id, class_id, section_id, school_classes(name), school_sections(name), academic_sessions(name), status)",
        )
        ...
        <CardContent className="p-4">
          <div className="relative mb-4">
            ...
            <Input
              ...
              placeholder="Type scholar number or student name..."
              value={q}
          ...
          <Table>
            ...
            <TableBody>
              {!q.trim() ? (
                ...
                results.data.map((s) => {
                  const active =
                    s.student_academic_records?.find((r) => r.status === "Active") ??
                  ...
                  return (
                    <TableRow
                      ...
                      onClick={() =>
                        nav({ to: "/fees/collect/$studentId", params: { studentId: s.id } })
                      }
                    >
