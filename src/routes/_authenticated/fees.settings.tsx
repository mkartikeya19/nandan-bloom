...
function FeeSettingsPage() {
  ...
  useEffect(() => {
    if (data) {
      ...
      setCollectionMode(((data as { default_collection_mode?: string }).default_collection_mode ?? "auto") as "auto" | "manual" | "ask");
    }
  }, [data]);

  const save = useMutation({
    mutationFn: async () => {
      const payload = { late_fee_enabled: enabled, late_fee_amount: amount, late_fee_grace_days: days, default_collection_mode: collectionMode };
      if (!data) {
    ...
    onSuccess: () => { toast.success("Saved"); qc.invalidateQueries({ queryKey: ["fee-settings"] }); qc.invalidateQueries({ queryKey: ["fee-settings-mode"] }); },
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 max-w-xl">
        <Card>
          <CardHeader><CardTitle className="text-base">Default Collection Mode</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            <p className="text-xs text-muted-foreground">Controls which allocation mode opens by default in Collect Payment.</p>
            <div className="flex flex-wrap gap-2">
              <Button size="sm" variant={collectionMode === "auto" ? "default" : "outline"} disabled={!canManageFeeStructures} onClick={() => setCollectionMode("auto")}>Automatic (Quick Collect)</Button>
              <Button size="sm" variant={collectionMode === "manual" ? "default" : "outline"} disabled={!canManageFeeStructures} onClick={() => setCollectionMode("manual")}>Manual Allocation</Button>
              <Button size="sm" variant={collectionMode === "ask" ? "default" : "outline"} disabled={!canManageFeeStructures} onClick={() => setCollectionMode("ask")}>Ask Every Time</Button>
            </div>
          </CardContent>
        </Card>
        ...
        {canManageFeeStructures && <Button onClick={() => save.mutate()} disabled={save.isPending} className="w-fit">{save.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save</Button>}
      </div>
    </div>
  );
}
