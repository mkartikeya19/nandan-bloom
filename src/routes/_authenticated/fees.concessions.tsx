...
function ConcessionsPage() {
  ...
  const [form, setForm] = useState({
    ...
    academic_session_id: "",
    fee_head_id: "",
    concession_type: "Principal Approved",
    reason: "",
    amount: "",
  });
    ...
    queryFn: async () =>
      (
        await supabase
          ...
          .select(
            "*, students(full_name, scholar_number), academic_sessions(name), fee_heads(name)",
          )
  ...
  const students = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("students")
          .select("id, scholar_number, full_name")
          .order("full_name")
          .limit(500)
      ).data ?? [],
  });
  ...
  const create = useMutation({
    mutationFn: async () => {
      if (!form.student_id || !form.academic_session_id || !form.amount)
  ...
  return (
    <div>
      <PageHeader
        title="Concessions"
        description="Approved fee concessions and audit trail."
        actions={
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => setForm({ ...form, concession_type: v })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <div className="space-y-1.5">
                    <Label>Fee Head (optional scope)</Label>
                    <Select
                      value={form.fee_head_id}
                      onValueChange={(v) => setForm({ ...form, fee_head_id: v })}
                    >
                      <SelectTrigger>
            ...
            <TableBody>
              {list.data?.length ? (
                list.data.map((c) => (
                  <TableRow key={c.id}>
                    <TableCell>
                      {new Date(c.approved_on ?? c.created_at).toLocaleDateString("en-IN")}
                    </TableCell>
                    <TableCell>
                      {c.students?.full_name}{" "}
                      <span className="text-xs text-muted-foreground">
                        ({c.students?.scholar_number})
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="font-semibold">
                      {c.amount != null ? formatINR(Number(c.amount)) : `${c.percentage}%`}
                    </TableCell>
