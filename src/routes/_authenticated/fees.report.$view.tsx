...
const TITLES: Record<View, { title: string; description: string }> = {
  pending: {
    title: "Students with Pending Fee",
    description: "Active students who still owe fees for the current session.",
  },
  ...
  outstanding: {
    ...
    description: "All active students with outstanding balances.",
  },
...
function FeeReportPage() {
  ...
  const view = (
    ["pending", "today", "month", "outstanding", "receipts"].includes(rawView) ? rawView : "pending"
  ) as View;
    ...
    queryFn: async () => {
      if (isReceipts) {
        let query = supabase
          .from("fee_payments")
          .select(
            "id, receipt_number, amount, payment_mode, payment_date, is_void, students(id, scholar_number, full_name, father_name)",
          )
          .eq("is_void", false)
      ...
      const { data: sched, error } = await supabase
        ...
        .select(
          "student_id, due_amount, concession_amount, paid_amount, students!inner(id, scholar_number, full_name, father_name, father_mobile, mother_mobile, guardian_phone, status, student_academic_records(academic_session_id, status, school_classes(name), school_sections(name), opening_balance))",
        )
      ...
      ((sched as any[]) ?? []).forEach((r) => {
        ...
        const active =
          (s.student_academic_records ?? []).find(
            (a: { status: string }) => a.status === "Active",
          ) ?? s.student_academic_records?.[0];
        const outstanding = Math.max(
          0,
          Number(r.due_amount ?? 0) - Number(r.concession_amount ?? 0) - Number(r.paid_amount ?? 0),
        );
  ...
  const studentIds = useMemo(
    () =>
      !isReceipts && rows.data
        ? (rows.data as Array<{ student_id: string }>).map((r) => r.student_id)
        : [],
    [isReceipts, rows.data],
  );
  const lastPayments = useQuery({
    ...
    queryFn: async () => {
      ...
      (data ?? []).forEach((p) => {
        if (!map.has(p.student_id)) map.set(p.student_id, p.payment_date);
      });
      return map;
    },
  });
  ...
  const filtered = useMemo(() => {
    ...
    return (rows.data as any[]).filter((r) => {
      ...
      return [r.full_name, r.scholar_number, r.father_name, r.class_name, r.mobile].some((v) =>
        String(v ?? "")
          .toLowerCase()
          .includes(needle),
      );
  ...
  const total = useMemo(() => {
    ...
    if (isReceipts)
      return (filtered as Array<{ amount: number | string }>).reduce(
  ...
  const exportCsv = () => {
    ...
    if (isReceipts) {
      ...
      body = (filtered as any[]).map((r) => [
        ...
        String(r.pending),
        lastPayments.data?.get(r.student_id) ?? "",
      ]);
    }
        ...
        <CardContent className="p-3 flex flex-col md:flex-row items-stretch md:items-center gap-3">
          ...
          <div className="text-sm">
            ...
            <Badge variant="secondary">
              {isReceipts
                ? formatINR(total)
                : `${filtered.length} students · ${formatINR(total)} pending`}
            </Badge>
          </div>
        </CardContent>
      </Card>
                  ...
                  isReceipts ? (
                    ...
                    <TableRow key={r.student_id}>
                      ...
                      <TableCell className="text-xs">
                        {lastPayments.data?.get(r.student_id)
                          ? new Date(lastPayments.data.get(r.student_id)!).toLocaleDateString(
                              "en-IN",
                            )
                          : "—"}
                      </TableCell>
