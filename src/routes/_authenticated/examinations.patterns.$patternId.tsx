...
function PatternEditor() {
  ...
  const classes = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("school_classes").select("id, name").order("sort_order")).data as
        | ClassRow[]
        | null,
  });
  ...
  const totalWeight = useMemo(
    () =>
      (terms.data ?? [])
        .filter((t) => t.include_in_final)
        .reduce((s, t) => s + Number(t.weightage_percent), 0),
    [terms.data],
  );
    ...
    <div>
          ...
          <div className="flex gap-2 items-center">
            {locked && (
              <Badge variant="secondary" className="gap-1">
                <Lock className="h-3 w-3" /> Locked — create a new version to edit
              </Badge>
            )}
            <Button asChild variant="outline" size="sm">
              <Link to="/examinations/patterns">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Select
                value={pattern.data?.grade_scale_id ?? ""}
                onValueChange={(v) => setGradeScale.mutate(v || null)}
                disabled={!canEdit}
              >
                <SelectTrigger>
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            <CardTitle className="text-base">Applicable Classes</CardTitle>
          </CardHeader>
          <CardContent>
            <ClassesGrid
              patternId={patternId}
              classes={classes.data ?? []}
              assigned={pclasses.data ?? []}
              canEdit={canEdit}
            />
          </CardContent>
        </Card>
...
function TermsTable({
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("exam_pattern_terms").insert({
        pattern_id: patternId,
        name: name.trim(),
        weightage_percent: weight,
        include_in_final: inFinal,
    ...
    onSuccess: () => {
      ...
      setWeight(0);
      setInFinal(true);
      qc.invalidateQueries({ queryKey: ["exam-pattern-terms", patternId] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
          ...
          {terms.length ? (
            terms.map((t) => (
              <TableRow key={t.id}>
                ...
                <TableCell>
                  {t.include_in_final ? <Badge>Yes</Badge> : <Badge variant="outline">No</Badge>}
                </TableCell>
                {canEdit && (
                  <TableCell>
      ...
      {canEdit && (
        <div className="flex flex-wrap gap-2 items-end pt-2 border-t">
          ...
          <label className="flex items-center gap-2 text-xs">
            <Switch checked={inFinal} onCheckedChange={setInFinal} /> Include in Final
          </label>
  ...
  const toggle = useMutation({
    mutationFn: async (classId: string) => {
      if (assignedSet.has(classId)) {
        ...
        const { error } = await supabase
          .from("exam_pattern_classes")
          .insert({ pattern_id: patternId, class_id: classId });
        if (error) throw error;
      }
    },
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {classes.length === 0 && (
        <p className="text-sm text-muted-foreground">
          No classes defined. Add classes in Settings.
        </p>
      )}
    </div>
  );
