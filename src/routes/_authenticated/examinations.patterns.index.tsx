...
function PatternsListPage() {
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase
        .from("exam_patterns")
        .insert({ name: newName.trim(), academic_session_id: newSession })
        .select("id")
        .single();
      if (error) throw error;
    ...
    onSuccess: (id) => {
      ...
      qc.invalidateQueries({ queryKey: ["exam-patterns"] });
      nav({ to: "/examinations/patterns/$patternId", params: { patternId: id } });
    },
  ...
  const clone = useMutation({
    mutationFn: async () => {
      ...
      ).rpc("clone_exam_pattern", {
        _source_id: cloneOpen.id,
        _new_session_id: cloneSession,
        _new_name: cloneName || cloneOpen.name,
      });
      if (error) throw error;
      return data;
    },
    onSuccess: (id) => {
      ...
      qc.invalidateQueries({ queryKey: ["exam-patterns"] });
      if (id) nav({ to: "/examinations/patterns/$patternId", params: { patternId: id } });
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div>
        ...
        actions={
          canManageExams && (
            <Dialog open={openNew} onOpenChange={setOpenNew}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>New Exam Pattern</DialogTitle>
                </DialogHeader>
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Primary Term Pattern"
                    />
                  </div>
                </div>
                <DialogFooter>
                  <Button onClick={() => create.mutate()} disabled={create.isPending}>
                    {create.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Create
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
              ...
              {patterns.data?.length ? (
                patterns.data.map((p) => (
                  <TableRow key={p.id}>
                    <TableCell>
                      <Link
                        to="/examinations/patterns/$patternId"
      ...
      <Dialog open={!!cloneOpen} onOpenChange={(o) => !o && setCloneOpen(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Clone Pattern</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
          ...
          <DialogFooter>
            <Button onClick={() => clone.mutate()} disabled={clone.isPending || !cloneSession}>
              {clone.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Clone
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
