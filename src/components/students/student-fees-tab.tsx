...
export function StudentFeesTab({ studentId, activeRecordId, hasFeeStructure = true }: Props) {
  ...
  const schedule = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, fee_head_id, period_label, period_month, period_year, due_amount, concession_amount, paid_amount, status, is_opening_balance, display_order, sort_key, fee_heads(name)",
        )
  ...
  const payments = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("fee_payments")
        .select(
          "id, receipt_number, amount, payment_mode, payment_date, is_void, transaction_reference",
        )
        .eq("student_id", studentId)
  ...
  const linkFeeStructure = useMutation({
    ...
    onSuccess: (count) => {
      toast.success(
        `Fee Structure linked. ${count} schedule row${count === 1 ? "" : "s"} generated.`,
      );
        ...
        <Card className="border-destructive/40 bg-destructive/5">
          <CardContent className="p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-destructive">
              This active academic record has no Fee Structure linked.
            </p>
            {canRepairFeeStructure && (
              <Button
                size="sm"
                onClick={() => linkFeeStructure.mutate()}
                disabled={linkFeeStructure.isPending}
              >
                {linkFeeStructure.isPending ? (
                  ...
                  {rows.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-6 text-muted-foreground">
                        No fee schedule yet.{" "}
                        {activeRecordId
                          ? "Open Fee Collection to generate one."
                          : "No active academic record."}
                      </TableCell>
                    </TableRow>
                  ) : (
                    rows.map((r) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="text-right">
                            {formatINR(r.concession_amount)}
                          </TableCell>
                          <TableCell className="text-right">{formatINR(r.paid_amount)}</TableCell>
                          <TableCell
                            className={
                              "text-right " + (os > 0 ? "font-semibold text-destructive" : "")
                            }
                          >
                            {formatINR(os)}
                          </TableCell>
  ...
  return (
    <Card>
      <CardContent className="p-4">
        ...
        <p
          className={"text-lg font-semibold mt-1 " + (tone === "danger" ? "text-destructive" : "")}
        >
          {value}
        </p>
        {action}
