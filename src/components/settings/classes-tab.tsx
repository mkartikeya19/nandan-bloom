...
export function ClassesTab({ canEdit }: { canEdit: boolean }) {
  ...
  const classesQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("school_classes").select("*").order("order_index");
      if (error) throw error;
      return (data ?? []) as Klass[];
    },
  });

  const sessionsById = useMemo(() => Object.fromEntries((sessionsQ.data ?? []).map((s) => [s.id, s])), [sessionsQ.data]);
  ...
  const openEdit = (k: Klass) => { setEditing(k); setForm({ session_id: k.session_id, name: k.name, order_index: k.order_index }); setOpen(true); };
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success(editing ? "Class updated" : "Class created"); setOpen(false); qc.invalidateQueries({ queryKey: ["school_classes"] }); },
    onError: (e: Error) => toast.error(e.message.includes("duplicate") ? "A class with that name already exists in this session" : e.message),
  });

  const del = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("school_classes").delete().eq("id", id); if (error) throw error; },
    onSuccess: () => { toast.success("Class deleted"); qc.invalidateQueries({ queryKey: ["school_classes"] }); },
      ...
      <CardHeader className="flex-row items-start justify-between gap-4">
        ...
        {canEdit && (
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button onClick={openNew} disabled={!sessionsQ.data?.length}><Plus className="h-4 w-4" /> New class</Button></DialogTrigger>
            <DialogContent>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select value={form.session_id} onValueChange={(v) => setForm({ ...form, session_id: v })}>
                    ...
                    <SelectContent>
                      {(sessionsQ.data ?? []).map((s) => <SelectItem key={s.id} value={s.id}>{s.name}{s.is_active ? " (active)" : ""}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2"><Label>Class name *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Class 1, Nursery, XII-Science" /></div>
                <div className="space-y-2"><Label>Display order</Label><Input type="number" value={form.order_index} onChange={(e) => setForm({ ...form, order_index: Number(e.target.value) || 0 })} /></div>
              </div>
              <DialogFooter>
        ...
        <div className="mb-4 flex flex-col sm:flex-row gap-2">
          ...
          <Select value={sessionFilter} onValueChange={setSessionFilter}>
            ...
            <SelectContent>
              ...
              {(sessionsQ.data ?? []).map((s) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>

        {classesQ.isLoading ? <Skeleton className="h-40 w-full" /> : (
          <div className="rounded-md border">
            <Table>
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  <TableRow><TableCell colSpan={canEdit ? 4 : 3} className="text-center text-sm text-muted-foreground py-8">No classes found</TableCell></TableRow>
                ) : filtered.map((k) => (
                  <TableRow key={k.id}>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>This will also delete all sections under this class.</AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>Cancel</AlertDialogCancel>
                              <AlertDialogAction onClick={() => del.mutate(k.id)}>Delete</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </TableCell>
