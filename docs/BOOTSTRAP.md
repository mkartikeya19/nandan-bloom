...
| 4 | Verify | Same person | Reload the app: the sidebar must show **Settings** and **Teachers** (Super Admin only). In Settings → Users the account must list roles *Super Admin* and *Admin*. |
...
2. **Academic session** — create and set exactly one session to *Active*.
